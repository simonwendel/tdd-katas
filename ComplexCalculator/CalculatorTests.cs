...
public class CalculatorTests
{
    ...
    private readonly Complex first = new(3, 2.5);
    private readonly Complex second = new(1.5, 3.05);
    ...
    public void Add_GivenTwoComplexNumbers_ReturnsNumberWithImAsSumOfImParts()
        => sut.Add(first, second).Im.Should().BeApproximately(5.55, Complex.Epsilon);

    [Fact]
    public void Add_GivenTwoComplexNumbers_ReturnsNumberWithReAsSumOfReParts()
        => sut.Add(first, second).Re.Should().BeApproximately(4.5, Complex.Epsilon);

    [Fact]
    public void Sub_GivenTwoComplexNumbers_ReturnsNumberWithImAsDiffOfImParts()
        => sut.Sub(first, second).Im.Should().BeApproximately(-0.55, Complex.Epsilon);

    [Fact]
    public void Sub_GivenTwoComplexNumbers_ReturnsNumberWithReAsDiffOfReParts()
        => sut.Sub(first, second).Re.Should().BeApproximately(1.5, Complex.Epsilon);
}